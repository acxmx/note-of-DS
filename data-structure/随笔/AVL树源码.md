...
typedef struct tree_n_t {
	int key；　struct tree_n_t *left,*right；　int height；
}
tree_node_t；　//建立堆栈　
tree_node_t *stack[MAXSIZE];
//warning!the tree can contain 256 leaves at most!　
int i=0;
//堆栈计数器　//堆栈清空　
void stack_clear() {
	while(i!=0) {
...
int rebalance(tree_node_t *node) {
	...
	while(!stack_empty()&&!finished) {
		...
		node=pop();
		//back to the root along the search path
		...
		if(node->left->height-node->right->height==2） {
			...
			node->height=node->left->height+1; else
			node->height=node->right->height+1;
		}
...
obj_node_t *find(tree_node_t *tree,int query_key) {
	...
	return(NULL); else {
		...
		return((obj_node_t*)tmp->left); else
		return(NULL);
	}
}
//建立插入函数
...
int del(tree_node_t *tree,int key) {
	...
	if(tree->left==NULL)
	return(-1）； else if(tree->right==NULL) {
		if(tree->key==key) {
			tree->left=NULL;
			return(0);
		} else
...
int travel(tree_node_t *tree) {
	stack_clear();
	if(tree->left==NULL)
	push(tree); else if(tree->left!=NULL) {
		int m=0;
		push(tree);
		while(i!=m) {
...
int test_structure(tree_node_t *tree) {
	...
	while(!stack_empty()) {
		...
		state=0; else if(stack->left!=NULL && stack->right!=NULL) {
...
void main() {
	...
	obj_node_t obj[256],*f;
	//MAXSIZE=n(number of leaf)+(n-1） number of node
	int n,j=0;
